...
fn temp_file(name: &str) -> PathBuf {
    std::env::temp_dir().join(format!("clawhdf5_test_{name}"))
}
...
mod mmap_tests {
    ...
    fn mmap_dataset_shape_dtype() {
        ...
        assert_eq!(ds.dtype().unwrap(), clawhdf5::DType::F64);
    ...
    fn mmap_nonexistent_file() {
        let result = MmapFile::open("/tmp/clawhdf5_nonexistent_12345.h5");
...
mod parallel_tests {
    ...
    fn parallel_matches_sequential() {
        ...
        let _sequential: Vec<Vec<u8>> = chunk_infos
            ...
            .map(|ci| {
                ...
                clawhdf5_format::filters::decompress_chunk(raw, &pipeline, chunk_bytes, elem_size)
                    .unwrap()
            })
            .collect();
        ...
        {
            let parallel = clawhdf5_format::parallel_read::decompress_chunks_parallel(
                &file_data,
                &chunk_infos,
                &pipeline,
                chunk_bytes,
    ...
    fn parallel_threshold() {
        assert!(!clawhdf5_format::parallel_read::should_use_parallel(1));
        assert!(!clawhdf5_format::parallel_read::should_use_parallel(4));
        assert!(clawhdf5_format::parallel_read::should_use_parallel(5));
        assert!(clawhdf5_format::parallel_read::should_use_parallel(100));
    }
}
...
mod fast_compression_tests {
    ...
    fn cross_backend_roundtrip() {
        ...
        let decompressed = clawhdf5_filters::deflate_decompress_miniz(&compressed).unwrap();
    ...
    fn large_data_roundtrip() {
        ...
        let decompressed = clawhdf5_filters::deflate_decompress(&compressed, data.len()).unwrap();
        assert_eq!(decompressed, data);
    }
    ...
    #[test]
    fn empty_roundtrip() {
        let compressed = clawhdf5_filters::deflate_compress(&[], 6).unwrap();
        let decompressed = clawhdf5_filters::deflate_decompress(&compressed, 0).unwrap();
    ...
    fn compression_levels() {
        ...
        for level in [1, 6, 9] {
            let compressed = clawhdf5_filters::deflate_compress(&data, level).unwrap();
            let decompressed =
...
mod checksum_tests {
    ...
    fn fletcher32_roundtrip_via_filters() {
        ...
        use clawhdf5_format::filters::{compress_chunk, decompress_chunk};
...
mod parallel_metadata_tests {
    use clawhdf5::{AttrValue, DatasetSpec, File, create_datasets_parallel};
    use clawhdf5_format::chunked_write::ChunkOptions;
    use clawhdf5_format::error::FormatError;
    use clawhdf5_format::file_writer::{IndependentDatasetBuilder, finalize_parallel};
    use clawhdf5_format::metadata_index::{MetadataBlock, MetadataIndex, build_dataset_metadata};
    use clawhdf5_format::type_builders::make_f64_type;
